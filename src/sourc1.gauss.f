...
c-------------------------------------------------------
      ...
      read(*,*)synthout2 
c 	  z added - was imag_par.out file messing things up?
...
c   
      ...
      real*8 npstd
      pi=3.1415926536 
c npstd=no. of points in one standard dev.
      npstd=peri*npps/6
c normalisation factor for the gaussian      
      nrminv=sqrt(2*pi*npstd*npstd)
c centre time for the gaussian
      l0=(lsec+nint(peri/2))*npps+1
c K&C 1977 used dinv=3
      dinv=2.7
      ...
      t=i-l0
c gaussian pulse      
      arg=t*t/2/npstd/npstd
      val=exp(-arg)/nrminv
      if(val.lt.0.001)then
      ui(i)=0.
      else
      ui(i)=val
      end if
