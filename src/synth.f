...
c====================================================================== 
      ...
      dimension  f(3,3,6),a(3,6),e(6,6),einv(6,6)
      real*4 dm,aa,reinv
      real*8 q3
      complex ari
      common/layers/q3(6,2000),ari(3,3,2000),dm(300),aa(6,6,6,2000),
     +   reinv(6,6)
      dimension  p(3,3,0:2),polyq(0:6),abair(13)
      dimension  solu(2,6),rr(3),ss(3)
      complex  arb(3,3),ar(3,3),carg
c     SIZE OF al DETERMINES MAX NUMBER OF LAYERS --> line 277
      complex  ac(6,6)
c------------- everything involved with sub "elast" is real*4.
c     SIZE OF dm DETERMINES MAX NUMBER OF LAYERS -changed from 150 to 250 Petruska 07.01.20
...
c---------------------------------------------------------------------  
      call frank (l,p,q3(1,l),f,ifiso,ifsym)
...
c---------------------------------------------------------------------  
      call conti (nl,l,a,c,hv,q3(1,l),e,einv,aa(1,1,1,l))
...
c---------------------------------------------------------------------  
c     START LOOP ON FREQUENCIES
!$omp parallel do private(w1,ithread)
      do 800 ifre=1,nc

c!$         ithread=omp_get_thread_num()+1
c      print *,'doing ithrea=',ithread,ifre

      w1=wp*real(ifre-1)
      call dofreq(w1,nl,ifre)
...
c=====================================================================  
c                   subroutine to go through "frequency loop"
c=====================================================================
      subroutine dofreq(w1,nl,ifre)
      implicit real*8 (a-h,o-z)
      real*4 dm,aa,reinv
      real*8 q3
      complex ari
      common/layers/q3(6,2000),ari(3,3,2000),dm(300),aa(6,6,6,2000),
     +   reinv(6,6)
c     SIZE OF al DETERMINES MAX NUMBER OF LAYERS CHANGED from 150 to 250 PETRUSKA 07.01.20
      complex arb(3,3),ar(3,3),carg
      complex phase(6),al(300,6,6),als(6,6),ac(6,6)
      real*4 realpt,imagpt
c!$    include 'omp_lib.h'

c     Start loop on layers (same frequency)
      do 370 l=1,nl-1
  ...
  348 phase(n)=cexp(carg)
      ...
      al(l,i,k)=cmplx(0.,0.)
      do 350 n=1,6
  350 al(l,i,k)=al(l,i,k)+aa(i,n,k,l)*phase(n)
  352 continue
...
c---------------------------------------------------------------------- 
      ...
      do 2 j=1,3
      do 1 i=1,3
    1 ari(i,j,ifre)=cmplx(0.,0.)
    2 ari(j,j,ifre)=cmplx(1.,0.)

      call cgauss (ar,ari(1,1,ifre),3,3,3)
